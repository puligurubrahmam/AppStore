...
class AppStore extends Component {
  state = {activeId: tabsList[0].tabId, searchItem: ''}
  UpdateList = () => {
    const {activeId, searchItem} = this.state
  ...
  render() {
    const filteredList = this.UpdateList()
        ...
        <div className="card">
            ...
            {tabsList.map(eachItem => {
              return (
                <TabItem
                  ...
                  key={eachItem.tabId}
                  activeClass={eachItem.tabId === activeId}
                />
              )
            })}
          </ul>
          <ul className="apps-container">
            {filteredList.map(each => {
              return <AppItem key={each.appId} appDetails={each} />
            })}
          </ul>
        </div>
      </div>
